            ...
            if ~strcmp(obj.info.drawROI, 'off')
                ...
                for i = 3:size(folder2Mov,1)
                    try
                        ...
                        %     FieldNamesMask = fieldnames(LoadMask);
                        %     mask{i,1} = LoadMask.(FieldNamesMask{1});
                        % else
                            if strcmp(obj.info.drawROI, 'channel1')
                                ...
                                Frame = Load.Movie.tif.getFrame(append(Folder1(idx1).folder, filesep, Folder1(idx1).name),obj.info.TestFrame);
                            ...
                            elseif strcmp(obj.info.drawROI, 'channel2')
                                ...
                                Frame = Load.Movie.tif.getFrame(append(Folder2(idx2).folder, filesep, Folder2(idx2).name),obj.info.TestFrame);
                        ...
                        disp(append("Failed to draw ROI on movie ", num2str(i-2), ' out of ', num2str(size(folder2Mov,1)-2)));
                    end
                end
                    ...
                    for i = 3:size(folder2Mov,1)
                        try
                            ...
                            for j = 1:nFrames
                                ...
                                Frame1 = Load.Movie.tif.getFrame(append(Folder1(idx1).folder, filesep, Folder1(idx1).name),j);
                                Frame1(mask{i,1} == 0) = NaN;
                                t1.setDirectory(j);
                                t1.write(Frame1);

                                if obj.info.multiModal == 1
                                    Frame2 = Load.Movie.tif.getFrame(append(Folder2(idx2).folder, filesep, Folder2(idx2).name),j);
    ...
    %             save(filename,'allTraces');
          ...
          function results = CalculateLocError(obj)
            ...
            %   in 3D space. Returns all per-particle displacements and their averages.
                ...
                save(append(obj.path, filesep, 'ChannelLocalisationError_water.mat'), "results")
                      ...
                      for j = 1:size(Traces3Dshort,1)
                          ...
                          try
                              ...
                              waitbar(j./size(Traces3Dshort, 1), f, append('Combining phase & tracking - Movie ', num2str(i),...
                                  ' out of ', num2str(nfields), ' - Trace ', num2str(j), ' out of ', num2str(size(Traces3Dshort,1))));
