classdef MPTrackingMovieRotational < Core.MPLocMovie
    ...
    methods
        ...
        function trackParticle(obj,trackParam,q)
                ...
                while ~isempty(ToTrack) 
                    if ~isempty(NextFrame.dataNext)
                        ...
                        [TrackedData_data{end+1},TrackedData_maxid] = Core.trackingMethod.AddDataToTracked( ImPrev.data...
                            ,NextFrame.dataNext, NextFrame.PossibleNeighbours,TrackedData_maxid);
        ...
        function correctDrift(obj)
            if strcmp(obj.info.CorrectDrift, 'on')
                if strcmp(obj.info.frame2Load, 'all')
                    nParticles = mean(obj.particles.nParticles);
                else
                    nParticles = mean(obj.particles.nParticles(obj.info.frame2Load));
                end

                if nParticles > 10
                    h = waitbar(0, 'initializing');
                    for i = 1:max(obj.particles.SRList.t)
                        waitbar(i./max(obj.particles.SRList.t), h, append('Correcting drift - frame ', num2str(i), ' out of ', num2str(max(obj.particles.SRList.t))));
                        CurrPartIdx = obj.particles.SRList.t == i;
                        CurrFramePart = obj.particles.SRList(CurrPartIdx == 1, :);
                        if ~isempty(CurrFramePart)
                            if i == 1
                                RefPositions = [mean(CurrFramePart.row), mean(CurrFramePart.col), mean(CurrFramePart.z)];
                            else
                                CurrPositions = [mean(CurrFramePart.row), mean(CurrFramePart.col), mean(CurrFramePart.z)];
                                Correction = RefPositions - CurrPositions;
                                obj.particles.SRList.row(CurrPartIdx) = CurrFramePart.row + Correction(1);
                                obj.particles.SRList.col(CurrPartIdx) = CurrFramePart.col + Correction(1);
                                obj.particles.SRList.z(CurrPartIdx) = CurrFramePart.z + Correction(1);
                            end
                        else
                            disp(append('No particles found in frame ', num2str(i)));
                        end
                    end
                    close(h)
                    disp('Driftcorrection succesfull')
                else
                    disp('Couldnt do driftcorrection - not enough particles per frame to average')
                end
            end
        end
