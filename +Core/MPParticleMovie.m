        ...
        function getROIs(obj)
            ...
            function [candidateList] = planeConsolidation(obj,candMet,focusMetric,consThresh,q)
                ...
                if strcmp(obj.info.Dimension, '2D')
                    currentCand = candMet(1,:);
                    nCols = size(currentCand, 2);
                    numericBlock1 = array2table(nan(5, nCols - 4), 'VariableNames', strcat("N1_", string(1:(nCols - 4))));
                    cellBlock = cell2table(cell(5, 2), 'VariableNames', strcat("C_", string(1:2)));
                    numericBlock2 = array2table(nan(5, 2), 'VariableNames', strcat("N2_", string(1:2)));
                    particleEmpty = [numericBlock1, numericBlock2, cellBlock];
                    particleEmpty.Properties.VariableNames = currentCand.Properties.VariableNames;

                    for i = 1:size(candMet, 1)                     
                        ParticleFull = particleEmpty;
