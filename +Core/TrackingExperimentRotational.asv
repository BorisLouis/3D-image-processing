...
%             save(filename,'allTraces');
        ...
        function ConsolidateChannels3(obj)
            ...
            for i = 1:size(closest_indices, 1)
                if closest_indices(i,3) <= obj.info.euDist
        ...
        function RotationalCalibration(obj)
            ...
            for i = 1:size(obj.traces3Dcommon,1)
                ...
                IntTot = mean(TotInt); 
                obj.traces3Dcommon{i,9} = TotInt;
                obj.traces3Dcommon{i,10} = IntTot;
            ...
            for i = 1:size(obj.traces3Dcommon,1)
                ...
                if ~any(FitRSquare(i,:) < 0.65)
                    ...
                    coeff = coeffvalues(f);
                    obj.traces3Dcommon{i,7} = abs(coeff(1));
                    obj.traces3Dcommon{i,8} = abs(coeff(1))./CorrFactor;
                    ...
                    obj.traces3Dcommon{i, 11} = obj.traces3Dcommon{i,10}/CorrFactor;
                else
                    obj.traces3Dcommon{i,7} = NaN;
                    obj.traces3Dcommon{i,8} = NaN;
                    obj.traces3Dcommon{i,9} = NaN;
                    obj.traces3Dcommon{i,10} = NaN;
                    obj.traces3Dcommon{i,11} = NaN;
                end      
            end
            close all
