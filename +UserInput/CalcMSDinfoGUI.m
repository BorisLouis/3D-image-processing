function [FilePath, Experiment, Filename, Dimension, ExpTime, Temp, Radius, Radius2, DiffFit, MinSize, Ext, ParticleType, path2RotCal, CutTraces, ExpModal, StepsizeAnalysis] = CalcMSDinfoGUI()
    ...
    fig = uifigure('Name', 'Parameter Input', ...
                   'Position', [100 100 450 750]);

    y = 700; dy = 40;

    % FilePath
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'File Path:');
    editFilePath = uieditfield(fig, 'text', 'Position', [150 y 260 22], 'Value', 'F:\Polymer Dynamics\20251004\PAA_1x_bAA\time3');

    % Experiment
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'Experiment:');
    dropdownExperiment = uidropdown(fig, 'Position', [150 y 260 22], ...
        'Items', {'Tracking', 'Tracking-Segmentation', 'Tracking-Phase', 'Rotational Tracking', 'Dual color tracking'}, ...
        'Value', 'Dual color tracking');

    % Filename
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'Filename:');
    dropdownFilename = uidropdown(fig, 'Position', [150 y 260 22],...
    ...
    % Dimension
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'Dimension:');
    dropdownDimension = uidropdown(fig, 'Position', [150 y 260 22], ...
        'Items', {'2D', '3D'}, 'Value', '3D');
    ...
    % Temp
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'Temp (K):');
    editTemp = uieditfield(fig, 'numeric', 'Position', [150 y 260 22], 'Value', 296.15);
    ...
    % DiffFit
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'DiffFit:');
    editDiffFit = uieditfield(fig, 'numeric', 'Position', [150 y 260 22], 'Value', 4);

    % MinSize
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'MinSize:');
    editMinSize = uieditfield(fig, 'numeric', 'Position', [150 y 260 22], 'Value', 20);

    % CutTraces
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'CutTraces:');
    editCutTraces = uieditfield(fig, 'numeric', 'Position', [150 y 260 22], 'Value', 50);

    % Extension
    y = y - dy;
    uilabel(fig, 'Position', [20 y 120 22], 'Text', 'Extension:');
    dropdownExt = uidropdown(fig, 'Position', [150 y 260 22], ...
        'Items', {'.mat', '.csc', '.xlsx'}, 'Value', '.mat');

    % StepsizeAnalysis (new)
    y = y - dy;
    ...
    % ParticleType (hidden)
    y = y - dy;
    labelParticleType = uilabel(fig, 'Position', [20 y 120 22], 'Text', 'Particle Type:', 'Visible', 'off');
    dropdownParticleType = uidropdown(fig, 'Position', [150 y 260 22], ...
        'Items', {'Bipyramid', 'ellipsoid', 'rod', 'cilinder'}, ...
        'Value', 'Bipyramid', 'Visible', 'off');

    % path2RotCal (hidden)
    y = y - dy;
    labelPath2RotCal = uilabel(fig, 'Position', [20 y 120 22], 'Text', 'path2RotCal:', 'Visible', 'off');
    editPath2RotCal = uieditfield(fig, 'text', 'Position', [150 y 260 22], 'Value', '', 'Visible', 'off');

    % ExpModal (hidden)
    y = y - dy;
    labelExpModal = uilabel(fig, 'Position', [20 y 120 22], 'Text', 'ExpModal:', 'Visible', 'off');
    dropdownExpModal = uidropdown(fig, 'Position', [150 y 260 22], ...
    ...
    % ---------------------------------------------------------
    % FIX: Apply visibility rules immediately based on default
    % ---------------------------------------------------------
    ...
    function submitCallback()
        ...
        StepsizeAnalysis = dropdownStepsize.Value;
        ...
        if strcmp(Experiment, 'Rotational Tracking')
            ...
            Radius = str2num(editRadiusArray.Value);
            Radius2 = NaN;
