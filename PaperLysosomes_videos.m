...
for s = 3:size(HeadFolders,1)
    % try
        ...
        % try
            ...
            mp4File_alpha = fullfile(mainFolder1,'TraceMovie_Alpha_bgParticles.avi');
            ...
            for j = 1:nTraces1
                ...
                for i = 1:size(currTrace, 1)
                    MaskFrame = Mask{currTrace.t(i), 1};
                    InSegmentList(i) = MaskFrame(round(currTrace.rowM(i)), round(currTrace.colM(i)));
                end
                if mean(InSegmentList) > 0.4
                    isInSegment(j) = mean(InSegmentList);
                else
                    isInSegment(j) = false;
                end
                ...
                if isempty(coord_nm) || size(coord_nm,1) < 5
                    Dvals(j) = NaN;
                    alphas(j) = NaN;
                else 
            ...
            axis(ax2,'off');
            text(ax2,0.5,0.7,'In segment','HorizontalAlignment','center','FontWeight','bold', 'Rotation', 90);
            text(ax2,0.5,0.3,'Out of segment','HorizontalAlignment','center','FontWeight','bold', 'Rotation', 90);
            ...
            % Video writer
            v_seg = VideoWriter(mp4File_seg,'Uncompressed AVI'); v_seg.FrameRate = frameRate*5; open(v_seg);
            ...
            v_diff = VideoWriter(mp4File_diff,'Uncompressed AVI'); v_diff.FrameRate = frameRate*5; open(v_diff);
            ...
            v_alpha = VideoWriter(mp4File_alpha,'Uncompressed AVI'); v_alpha.FrameRate = frameRate*5; open(v_alpha);
        ...
        try
            ...
            %% Video output names
            mp4File_seg = fullfile(mainFolder1,'TraceMovie_Segment_bgLysosomes.avi');
            mp4File_diff = fullfile(mainFolder1,'TraceMovie_Diffusion_bgLysosomes.avi');
            mp4File_alpha = fullfile(mainFolder1,'TraceMovie_Alpha_bgLysosomes.avi');
            ...
            if contains(CellName, 'A549')
                load('D:\MultiColor - lysosome tracking\raw data for presentation\2D files\2DCal_A549\2DCal.mat');
            elseif contains(CellName, 'HeLa')
                load('D:\MultiColor - lysosome tracking\raw data for presentation\2D files\2DCal_HeLa\2DCal.mat');
            else
                load('D:\MultiColor - lysosome tracking\raw data for presentation\2D files\2Dcal_HepG2_MCF_KM12\2DCal.mat');
            end

            for i = 1:size(mov1, 3)
                mov1(:,:,i) = imwarp(mov1(:,:,i), tform, "Outputview", imref2d(size(mov1(:,:,i))));
            end
            ...
            trace_lengths = cellfun(@(t) size(t,1), traces1(:,1));
            rows_to_keep = trace_lengths >= minSize;
            traces1 = traces1(rows_to_keep, :);
            ...
            for j = 1:nTraces1
                waitbar(j./nTraces1,h,append('Calc D and alpha: Trace ', num2str(j), ' out of ', num2str(nTraces1)));
                currTrace = traces1{j,1}; % table expected with columns 'row','col','z', ... 'InSegment'
                % --- determine InSegment majority ---
                for i = 1:size(currTrace, 1)
                    MaskFrame = Mask{currTrace.t(i), 1};
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%
         try
            ...
            %% Video output names
            mp4File_seg = fullfile(mainFolder1,'TraceMovie_Segment_bgParticles_maskLysosomes.avi');
            mp4File_diff = fullfile(mainFolder1,'TraceMovie_Diffusion_bgParticles_maskLysosomes.avi');
            mp4File_alpha = fullfile(mainFolder1,'TraceMovie_Alpha_bgParticles_maskLysosomes.avi');
