...
%   Detailed explanation goes here
    ...
    if or(isempty(cam1),isempty(cam2))
        ...
        if cal.multiModal == true
            ...
            [ chC3wrongdim, chC4wrongdim ] = mpSetup.cali.getChData( cam1, cam2, cal.ROI2FullCam );

            chC3 = [];
            chC4 = []; 
            for z = 1:size(chC3wrongdim, 3)
                for t = 1:size(chC4wrongdim,4)
                    chC3(:,:,z,t) = imresize(chC3wrongdim(:,:,z, t), size(chC1(:,:,z,t)));
                    chC4(:,:,z,t) = imresize(chC4wrongdim(:,:,z, t), size(chC1(:,:,z,t)));
                end
            end
