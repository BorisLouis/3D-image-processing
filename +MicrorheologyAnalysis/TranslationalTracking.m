classdef TranslationalTracking < handle
    ...
    methods
        ...
        function CalculateStepsizes(obj,Loop)
            ...
            elseif run == 1
                ...
                for k = 1:nRows
                    ...
                    if ~isempty(AllStepSizes)
                        Results{k,1} = allRes;
                        [N, edges] = histcounts(AllStepSizes(:,1), [0:0.65/50:0.65]);
                        Trend(k, :) = N./max(N);
                    else
                        Trend(k, :) = nan(1, 50);
                    end
                ...
                Fig = figure();
                imagesc(Trend)
                title('Normalized distribution of stepsizes')
                ylabel('Frame')
                saveas(Fig, append(obj.raw.Path, filesep, 'StepsizeDistribution.png'))
                saveas(Fig, append(obj.raw.Path, filesep, 'StepsizeDistribution.svg'))
            end
        ...
        function [p_fit, FinalTrend] = FitPopulationFractions(obj, Loop)
            ...
            TimeFinal = [x(3:end) - p_fit(3)]';
            DiffFinal = y;
            FinalTrend = [TimeFinal', DiffFinal];

            FitResults.Fit = array2table(p_fit, 'VariableNames', {'Base','Height','Inflection', 'Slope','Start drop'});
            FitResults.Fit.FullTrend = {FinalTrend};
