...
for i = 3:size(Folder, 1)
    try
        ...
        if Folder(i).isdir == 1
            ...
            VideoName = append(File, filesep, Folder(i).name, '_MMStack_Pos0.ome.tif');
            [ frameCell, movieInfo, tfl ] = Load.Movie.ome.getInfo( VideoName );
            [ movC1, movC2, idx ] = Load.Movie.ome.load( frameCell, movieInfo, 50);
            for ste = 1:4
                Frame(:,:,ste) = movC1(ROI1(ste,2)+20:ROI1(ste,2)+ROI1(ste,4)-20, ROI1(ste,1)+20:ROI1(ste,1)+ROI1(ste,3)-20);
                Frame(:,:,ste+4) = movC2(ROI1(ste,2)+20:ROI1(ste,2)+ROI1(ste,4)-20, ROI1(ste,1)+20:ROI1(ste,1)+ROI1(ste,3)-20);
            end
            for ste = 1:size(Frame, 3)
                Int(ste, 1) = mean(Frame(:,:,ste), 'all');
                GradVar(ste, 1) = mean(imgradient(Frame(:,:,ste), 'sobel'), 'all');
                LocalVar(ste, 1) = mean(stdfilt(Frame(:,:,ste), true(3)), 'all');
                SharpnessLaplacian(ste, 1) = mean(imfilter(Frame(:,:,ste), fspecial('laplacian', 0.2), 'replicate'), 'all');
            end
            if strcmp(File(end-4:end), 'n0__1')
                GradMag = [allRes.GradientMagnitude];
                allRes = allRes(GradMag < 0.5);
                Diff0 = [Diff0; mean([allRes.DR]')];
                a0 = [a0; mean([allRes.aR]')];
                n0 = [n0; mean([allRes.nR]')];
                ...
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase0 = [phase0; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase0 = [phase0; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase0 = [phase0; SharpnessLaplacian];
                end
            elseif strcmp(File(end-4:end), 'n2__1')
                Diff2 = [Diff2; mean([allRes.DR]')];
                a2 = [a2; mean([allRes.aR]')];
                n2 = [n2; mean([allRes.nR]')];
                if strcmp(Parameter, 'Phase')
                    phase2 = [phase2; [allRes.Phase]'];
                elseif strcmp(Parameter, 'IntPhaseCh')
                    phase2 = [phase2; Int];
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase2 = [phase2; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase2 = [phase2; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase2 = [phase2; SharpnessLaplacian];
                end
            elseif strcmp(File(end-4:end), 'n4__1')
                ...
                Diff4 = [Diff4; mean([allRes.DR]')];
                a4 = [a4; mean([allRes.aR]')];
                n4 = [n4; mean([allRes.nR]')];
                if strcmp(Parameter, 'Phase')
                    phase4 = [phase4; [allRes.Phase]'];
                elseif strcmp(Parameter, 'IntPhaseCh')
                    phase4 = [phase4; Int];
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase4 = [phase4; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase4 = [phase4; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase4 = [phase4; SharpnessLaplacian];
                end
            elseif strcmp(File(end-4:end), 'n6__1')
                ...
                Diff6 = [Diff6; mean([allRes.DR]')];
                a6 = [a6; mean([allRes.aR]')];
                n6 = [n6; mean([allRes.nR]')];
                if strcmp(Parameter, 'Phase')
                    phase6 = [phase6; [allRes.Phase]'];
                elseif strcmp(Parameter, 'IntPhaseCh')
                    phase6 = [phase6; Int];
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase6 = [phase6; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase6 = [phase6; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase6 = [phase6; SharpnessLaplacian];
                end
            elseif strcmp(File(end-4:end), 'n8__1')
                allDR = [allRes.DR];
                cutoff = prctile(allDR, 25);
                allRes = allRes(allDR > cutoff);
                Diff8 = [Diff8; mean([allRes.DR]')];
                a8 = [a8; mean([allRes.aR]')];
                n8 = [n8; mean([allRes.nR]')];
                if strcmp(Parameter, 'Phase')
                    phase8 = [phase8; [allRes.Phase]'];
                elseif strcmp(Parameter, 'IntPhaseCh')
                    phase8 = [phase8; Int];
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase8 = [phase8; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase8 = [phase8; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase8 = [phase8; SharpnessLaplacian];
                end
            elseif strcmp(File(end-4:end), '10__1')
                ...
                Diff10 = [Diff10; mean([allRes.DR]')];
                a10 = [a10; mean([allRes.aR]')];
                n10 = [n10; mean([allRes.nR]')];
                if strcmp(Parameter, 'Phase')
                    phase10 = [phase10; [allRes.Phase]'];
                elseif strcmp(Parameter, 'IntPhaseCh')
                    phase10 = [phase10; Int];
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase10 = [phase10; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase10 = [phase10; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase10 = [phase10; SharpnessLaplacian];
                end
            elseif strcmp(File(end-4:end), '13__1')
                Diff13 = [Diff13; mean([allRes.DR]')];
                a13 = [a13; mean([allRes.aR]')];
                n13 = [n13; mean([allRes.nR]')];
                if strcmp(Parameter, 'Phase')
                    phase13 = [phase13; [allRes.Phase]'];
                elseif strcmp(Parameter, 'IntPhaseCh')
                    phase13 = [phase13; Int];
                elseif strcmp(Parameter, 'GradientMagnitude')
                    phase13 = [phase13; GradVar];
                elseif strcmp(Parameter, 'LocalVariance')
                    phase13 = [phase13; LocalVar];
                elseif strcmp(Parameter, 'SharpnessLaplacian')
                    phase13 = [phase13; SharpnessLaplacian];
                end
            else
